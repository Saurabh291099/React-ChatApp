...
const Sidebar = ({ users, selectChat, markAsUnread, deleteChat, resetUnreadCount }) => {
        ...
        <div className='border-r border-[#EFEFEF] py-4 px-2 h-[100vh] '>
            ...
            <div className='flex flex-col gap-2'>
                {users.map(user => {
                    ...
                    return (
                        <>
                            <div className='h-16 p-2 bg-[#F5F7FB] rounded-md hover:cursor-pointer' key={user.userId} >
                                <div className='grid grid-cols-12 gap-2'>
                                    <div className="col-span-2 flex justify-center items-center w-12 h-12 rounded-full overflow-hidden" onClick={() => { selectChat(user.userId); resetUnreadCount(user.userId); }}>
                                        <img className='w-full h-full object-cover' src={user.profilePictureURL} alt={user.name} />
                                    </div>

                                    <div className="col-span-8" onClick={() => { selectChat(user.userId); resetUnreadCount(user.userId); }}>
